...
<ParamField body="Query Parameters" type="object" children="Optional additional query parameters.">
  <Expandable title="Toggle object">
    ...
    <ParamField body="mode" type="string">
      Set to `sync` to have results returned synchronously, instead of using async jobs. Note that complex prompts may result in timeouts or incomplete data when using this option.
    </ParamField>
...
<ResponseField name="requestUUID" type="string">
    The UUID of the request (job) which you can use to [poll status](./api-reference/endpoints/request-status).
...
Note: only present when `mode` is `sync`.
